...
JQX('jqx-tank', class Tank extends JQX.BaseElement {
    ...
    propertyChangedHandler(key, oldValue, value) {
        ...
        switch (key) {
            ...
            case 'max': {
                ...
                const numberToValidate = that._numericProcessor.createDescriptor(that._discardDecimalSeparator(that.value, that.decimalSeparator)),
    ...
    **/
    ...
    _validateMinMax(validatedProperty, initialValidation, oldValue) {
        ...
        function validator(param, oldValue) {
            ...
            let validateCondition = param === 'min' ? (new JQX.Utilities.BigNumber(that.max).compare(value) <= 0) : (new JQX.Utilities.BigNumber(that.min).compare(value) > 0);
    ...
    **/
    ...
    _getRange() {
        ...
        that._range = (new JQX.Utilities.BigNumber(that._drawMax).subtract(new JQX.Utilities.BigNumber(that._drawMin)).toString());
    }
    ...
    */
     ...
     */
    _validate(initialValidation, programmaticValue, coerced) {
        const that = this;
        let value;
    ...
    */
    ...
    _keyIncrementDecrement(action, preValue) {
        ...
        if (that._drawValue.compare(that._drawMin) === -1) {
            return new JQX.Utilities.BigNumber(that._drawMin);
        }

        if (that._drawValue.compare(that._drawMax) === 1) {
            return new JQX.Utilities.BigNumber(that._drawMax);
        }
    ...
    **/
    ...
    _coerceCustomInterval() {
        ...
        that._validate(false, newCoercedValue, true);
