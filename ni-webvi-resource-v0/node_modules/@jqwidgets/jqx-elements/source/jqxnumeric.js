...
JQX.Utilities.Assign('BaseNumericProcessor', class BaseNumericProcessor {
    ...
    drawGaugeLogarithmicScaleMinorTicks(majorTickValues, majorStep, drawMinor) {
        ...
        if (majorStep instanceof BigNumberNG) {
...
JQX.Utilities.Assign('IntegerNumericProcessor', class IntegerNumericProcessor extends JQX.Utilities.BaseNumericProcessor {
    ...
    createDescriptor(initialValue, supportsENotation, validateConstruction, validateMinMax, discardRadix) {
        ...
        if (initialValue.constructor !== BigNumberNG) {
            let radix = !discardRadix && that._radixNumber ? that._radixNumber : 10;

            if (radix === 10 && supportsENotation && initialValue.constructor !== BigNumberNG && this.isENotation(initialValue)) {
    ...
    */
    ...
    valueToPx(value) {
        ...
        if (that._normalLayout) {
            const drawMin = that._drawMin instanceof BigNumberNG ? that._drawMin : new JQX.Utilities.BigNumber(that._drawMin);

            value = new JQX.Utilities.BigNumber(value);
            result = parseFloat((lengthRangeRatio.multiply(value.subtract(drawMin))).toString());
        }
        else {
            const drawMax = that._drawMax instanceof BigNumberNG ? that._drawMax : new JQX.Utilities.BigNumber(that._drawMax);
    ...
    */
    ...
    updateToolTipAndValue(value, oldValue, changeValue) {
        ...
        value = value instanceof BigNumberNG ? value : new JQX.Utilities.BigNumber(value);
    ...
    updateValue(value) {
        ...
        if (!that._programmaticValueIsSet && (this.compare(value, that._number) || that._scaleTypeChangedFlag)) {
            that.$.fireEvent('change', { 'value': value, 'oldValue': oldValue });
        }
    ...
    getValuePerPx(range, pxRange) {
        return new JQX.Utilities.BigNumber(range).divide(pxRange);
    }
    ...
    restrictValue(values) {
        if (values[1].constructor === BigNumberNG) {
    ...
    getAngleByValue(value, calculateDrawValue, returnDegrees) {
        ...
        if (value instanceof BigNumberNG === false) {
            value = new JQX.Utilities.BigNumber(value);
        }
     ...
     */
    updateGaugeValue(newValue) {
        if (newValue instanceof BigNumberNG === false) {
            super.updateGaugeValue(newValue);
        }
        ...
        if (typeof moduloCoefficient === 'undefined') {
            ...
            if ((dividend < -1 || dividend > 1) && (divisor < -1 || divisor > 1 || divisor === 1)) {
                if (dividend % 1 === 0 && divisor % 1 === 0) {
                    ...
                    return sign * parseFloat(new JQX.Utilities.BigNumber(dividend).mod(divisor).toString());
                }
            }
   ...
   */
    ...
    updateValue(value) {
        const that = this.context,
            ...
            oldValue = that._number.toString();

        if (value.toString() !== oldValue.toString() || that._scaleTypeChangedFlag) {
            that.value = value.toString();
            that._number = renderedValue;
...
JQX.Utilities.Assign('NumberRenderer', class NumberRenderer {
    ...
    bigNumberToExponent(significantDigits, forceExponentialResult) {
        ...
        if (value.constructor !== BigNumberNG) {
            value = new JQX.Utilities.BigNumber(value);
        }
        ...
        if (numberLength <= 10) {
            ...
            if (forceExponentialResult) {
                return parseFloat(numericString).toExponential(significantDigits - 1);
            }
            else {
    ...
    toScientific() {
        ...
        if (that.numericValue._d) {
            return that.bigNumberToScientific(arguments[0], arguments[1]);
        }

        const exponentialValue = Number(that.numericValue.toString()).toExponential(),
    ...
    formatNumber(number, formatSpecifier, wordLength) {
        const that = this,
            format = formatSpecifier.slice(0, 1);
        ...
        switch (format) {
            ...
            case 'g': {
                ...
                let scientificRepresentation = that.formatNumber(number, 'E' + precision),
            ...
            case 'x': {
                ...
                const wordLengthNumber = numericProcessor.getWordLength(wordLength ? wordLength : 'int32');
                let radix;
                ...
                result = numericObject.toString(radix, wordLengthNumber);
