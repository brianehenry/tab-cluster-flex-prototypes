(function () {
    ...
    class ErrorModule {
        ...
        _logger(level, error) {
            ...
            if (that.debugMode) {
                let errorMessage = error instanceof Error ? error.message : error.toString();
    ...
    class BindingModule {
        ...
        _addRemovePropertyBinding(hostPropertyName, targetPropertyName, targetElement, removeBinding) {
            ...
            const boundProperty = { name: hostPropertyName, reflectToAttribute: hostProperty.reflectToAttribute, twoWay: twoWayBinding, type: hostProperty.type };
    ...
    class Ripple {
        static animate(element, left, top) {
            ...
            that.scrollTo = function (value, vertically) {
                ...
                const autoScroll = function () {
                    ...
                    if (isScrolling) {
                        ...
                        if (time >= duration) {
                            cancelAnimationFrame(rafId);
                            that.container.$.fireEvent('scrollComplete');
                            rafId = 0;
                            return;
                        }
    ...
    class Extend {
        ...
        get isCustomElement() {
            ...
            if (that.element instanceof window.JQX.BaseElement === true) {
                return true;
            }

            if (document.createElement(that.element.nodeName) instanceof window.JQX.BaseElement === true) {
                return true;
            }
    ...
    set(value) {
        ...
        setup() {
            ...
            const templateNodes = [].slice.call(that.querySelectorAll('[jqx-id]')).concat(children);
        ...
        getStaticMember(memberName) {
            const that = this;
            const element = window.JQX[that.elementName];

            let staticMember = element[memberName];
            let inheritedStaticMember = {};
        ...
        defineElementMethods(methods, proto) {
            ...
            const invokeMethod = function (method, methodName) {
                ...
                const elementMethod = function () {
                    ...
                    this.context = this;
                    const result = method.apply(methodContext, args.concat(Array.prototype.slice.call(arguments)));
  ...
  //  const isIE = navigator.userAgent.match(/MSIE/) || (!!window.MSInputMethodContext && !!document.documentMode);
    ...
    class BaseCustomElementV1 extends BaseElement {
        ...
        set externalStyle(path) {
            this._externalStylePath = path;
        }

        addExternalStylesheet(path) {
            const that = this;
    ...
    class ElementRegistry {
        ...
        static register(tagName, element) {
            ...
            const elementName = Core.toCamelCase(tagName).replace(/[a-z]+/, '');
        ...
        static registerElements() {
            ...
            for (let i = that.toRegister.length - 1; i >= 0; i--) {
    ...
    class ObservableArray {
        ...
        defineIndexProperty(index) {
            ...
            if (!(index in that)) {
                            ...
                            if (typeof that.notifier === 'function') {
                                ...
                                const getPath = function (obj, path) {
                                    Object.getOwnPropertyNames(obj).forEach(function (name) {
                                        ...
                                        let type = JQX.Utilities.Types.getType(obj[name]);
                ...
                const defineProperties = function (obj, path, context) {
                    ...
                    const type = JQX.Utilities.Types.getType(obj);
        ...
        slice(start, end) {
            ...
            const observableArray = new JQX.ObservableArray(newArray, that.observeFunction);
        ...
        splice(index, howMany) {
            ...
            const observableArray = new JQX.ObservableArray(removed, that.observeFunction);
            return observableArray;
        }
    
        fromArray(array) {
            const that = this;
            const observableArray = new JQX.ObservableArray(array, that.observeFunction);
        ...
        clone() {
            ...
            const observableArray = new JQX.ObservableArray(that.array, that.observeFunction);
        ...
        concat(args, observeFunction) {
            ...
            const observableArray = new JQX.ObservableArray(newArray, observeFunction);
        ...
        set(index, value) {
            ...
            if (JQX.Utilities.Types.getType(index) === 'string' && index.split('.').length > 1) {
    ...
    if (window.JQX) {
        userDefinedSettings = window.JQX;
    }
    ...
    let theme = window.JQX.Theme;

    Object.defineProperty(window.JQX, 'Theme', {
        configurable: false,
        enumerable: true,
        get() {
            return theme;
    ...
    * Content element.
    */
    window.JQX('jqx-content-element', class ContentElement extends window.JQX.BaseElement {
        static get properties() {
            return {
                'content': {
                    type: 'any',
    ...
    window.JQX('jqx-scroll-viewer', class ScrollViewer extends window.JQX.ContentElement {
        static get properties() {
            return {
        ...
        set scrollWidth(value) {
            ...
            if (that._scrollView && that._scrollView.hScrollBar) {
                if (value === 0 && that.horizontalScrollBarVisibility === 'visible') {
                    that._scrollView.hScrollBar.max = 1;
                }
                else {
