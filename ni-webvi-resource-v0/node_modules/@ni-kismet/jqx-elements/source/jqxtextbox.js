 ...
 */
    _checkBrowserBoundsVertically() {
        const that = this;
        let dropDownListCoordinates = that.getBoundingClientRect();
    ...
    _focusHandler(event) {
        ...
        if (event.type === 'blur') {
            ...
            if ((that.autoComplete === 'auto' || that.autoComplete === 'inline') && that.$.input.value.length > 0 &&
                that.$.listBox._focusedItem && that.$.listBox._focusedItem.focused) {
                that.$.input.value = that.$.listBox._focusedItem.label;
            }
    ...
    _keyDownHandler(event) {
        ...
        switch (event.key) {
            ...
            case 'PageDown':
                ...
                break;
            case 'Enter':
                if (that.opened && that.$.listBox._focusedItem && that.$.listBox._focusedItem.focused) {
                    that.$.listBox.$.filterInput.value = that.$.listBox._focusedItem.label;
                    that.$.input.value = that.displayMode === 'escaped' ?
                        that._toEscapedDisplayMode(that.$.listBox._focusedItem.label) : that._toDefaultDisplayMode(that.$.listBox._focusedItem.label);
    ...
    */
    _textBoxKeyUpHandler(event) {
        ...
        if (that.$.listBox.selectedIndexes.length === 1) {
            ...
            if (that.value !== selectedItem.label) {
        ...
        if (event.key === 'Alt' || event.key === 'Control' || (!that.opened && event.key === 'Escape')) {
   ...
   */
    _listBoxChangeHandler(event) {
        ...
        if (event.detail.selected) {
            that.$.listBox.$.filterInput.value = event.detail.label;
            that.$.input.value = that.displayMode === 'escaped' ?
                that._toEscapedDisplayMode(event.detail.value) : that._toDefaultDisplayMode(event.detail.label);
        }
    }
    ...
    */
    _mouseEventsHandler(event) {
        ...
        event.type === 'mouseenter' ? that.$.addClass('hovered') : that.$.removeClass('hovered');
    }
    ...
    */
    _documentUpHandler(event) {
        ...
        if (that._isListBoxThumbDragged || that.readonly) {
            return;
        }
        ...
        while (target) {
            if (target instanceof JQX.ListItem && target.ownerListBox === that.$.listBox) {
                ...
                if (that.displayMode === 'escaped') {
                    that.value = target.label;
                    that.$.input.value = that._toEscapedDisplayMode(target.label);
                }
                else {
                    that.$.input.value = that.value = target.label;
                }
