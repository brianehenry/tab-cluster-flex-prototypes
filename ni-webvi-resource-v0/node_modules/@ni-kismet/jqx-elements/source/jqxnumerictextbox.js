...
JQX('jqx-numeric-text-box', class NumericTextBox extends JQX.BaseElement {
        ...
        const template =
            '<div id="container" class="jqx-container">' +
                '<span id="label" inner-h-t-m-l="[[label]]" class="jqx-label"></span>' +
                    '<div id="radixDisplayButton" class="jqx-unselectable jqx-numeric-text-box-component jqx-numeric-text-box-radix-display"></div>' +
                    '<input id="input" type="text" class="jqx-numeric-text-box-component" placeholder="[[placeholder]]" readonly="[[readonly]]" disabled="[[disabled]]" name="[[name]]" />' +
                    '<div id="unitDisplay" class="jqx-unselectable jqx-numeric-text-box-component jqx-numeric-text-box-unit-display"></div>' +
                    '<div id="spinButtonsContainer" class="jqx-numeric-text-box-component jqx-spin-buttons-container">' +
                        '<jqx-repeat-button initial-delay="[[spinButtonsInitialDelay]]" delay="[[spinButtonsDelay]]" unfocusable id="upButton" class="jqx-spin-button">' +
                            '<div class="jqx-arrow jqx-arrow-up"></div>' +
                        '</jqx-repeat-button>' +
                        '<jqx-repeat-button initial-delay="[[spinButtonsInitialDelay]]" delay="[[spinButtonsDelay]]" unfocusable id="downButton" class="jqx-spin-button">' +
                            '<div class="jqx-arrow jqx-arrow-down"></div>' +
                        '</jqx-repeat-button>' +
                    '</div>' +
                    '<ul id="dropDown" class="jqx-hidden jqx-drop-down">' +
    ...
    _validate(initialValidation, programmaticValue) {
        ...
        const validNumber = that._numericProcessor.createDescriptor(validationOptions.value, true, true, programmaticValue === undefined, initialValidation || programmaticValue !== undefined, validationOptions.enteredComplexNumber);
    ...
    _openRadix() {
        ...
        that.$dropDown.removeClass('jqx-hidden');
    ...
    _closeRadix() {
        ...
        that.$dropDown.addClass('jqx-hidden');
    ...
    _documentMousedownHandler(event) {
        const that = this;
        if (that.opened && !that.contains(event.target)) {
            that._closeRadix();
        }
    }
    ...
    _inputBlurHandler() {
        ...
        else if (that.$.input.value !== that._cachedInputValue) {
            that._triggerChangeEvent = true;
            that._validate();
            that._triggerChangeEvent = false;
        }
    ...
    _setDropDownOptions() {
        ...
        if (that.dropDownEnabled === false) {
            return;
        }

        const wordLength = that._wordLengthNumber;

        that.$.dropDownItem2.innerHTML = `${that._number.toString(2, wordLength)} (${that.localize('binary')})`;
        ...
        that.$.dropDownItem16.innerHTML = `${that._number.toString(16, wordLength)} (${that.localize('hexadecimal')})`;
    }
     ...
     */
    _renderValue(renderedValue, ignoreRadixNumber) {
        const that = this;
