...
(function ($) {
    ...
    function init(plot) {
        ...
        plot.setCursor = function setCursor(cursor, options) {
            ...
            if (index !== -1) {
                ...
                setPosition(plot, cursors[index], cursors[index].position, positionMode);
                plot.triggerRedrawOverlay();
            }
        };
        ...
        var thumbmove = {
            ...
            drag: function drag(e) {
                ...
                if (currentlySelectedCursor) {
                    ...
                    updateThumbsPosition(plot, currentlySelectedCursor, plot.getPlotOffset());
                }
            }
        };
        ...
        function switchToRelativePlotPosition() {
            ...
            cursors.forEach(function (cursor) {
                ...
                if (!isNaN(relativePosition.relativeX) && !isNaN(relativePosition.relativeY)) {
        ...
        plot.hooks.drawOverlay.push(function (plot, ctx) {
            var isMousePositionInitilized = function isMousePositionInitilized(mousePosition) {
            ...
            cursors.forEach(function (cursor) {
                ...
                initializeAxisPositionIfNecessary(plot, cursor, positionMode);
                setPosition(plot, cursor, cursor.position, positionMode);
                ...
                maybeSnapToPlot(plot, cursor, intersections, positionMode);
        ...
        function updateThumbsPosition(plot, cursor, plotOffset) {
            var cursorX, cursorY, yThumbIndex, thumb;
            if (xAxisThumb(cursor)) {
                thumb = cursor.thumbs[0];
                if (cursorInPlotXRange(plot, cursor)) {
            ...
            if (yAxisThumb(cursor)) {
                ...
                if (cursorInPlotYRange(plot, cursor)) {
    ...
    function initializeAxisPositionIfNecessary(plot, cursor, positionMode) {
        var xaxis = findXAxis(plot, cursor),
            yaxis = findYAxis(plot, cursor),
            position = cursor.position;
        if (positionMode === positionModes.axis && (position.x === undefined || position.y === undefined) && xaxis.c2p && yaxis.c2p) {
     ...
     */
    function setPosition(plot, cursor, pos, positionMode) {
        var xaxis = findXAxis(plot, cursor),
            yaxis = findYAxis(plot, cursor),
            x,
            y;
    ...
    function maybeSnapToPlot(plot, cursor, intersections, positionMode) {
    ...
    function cursorInPlotXRange(plot, cursor) {
        return cursor.x >= 0 && Math.floor(cursor.x) <= plot.width();
    }

    function cursorInPlotYRange(plot, cursor) {
        return cursor.y >= 0 && Math.floor(cursor.y) <= plot.height();
    }

    function cursorInPlotRange(plot, cursor) {
        return cursorInPlotXRange(plot, cursor) && cursorInPlotYRange(plot, cursor);
    }
    ...
    function drawVerticalAndHorizontalLines(plot, ctx, cursor) {
        ...
        if (cursor.mode.indexOf("x") !== -1 && cursorInPlotXRange(plot, cursor)) {
        ...
        if (cursor.mode.indexOf("y") !== -1 && cursorInPlotYRange(plot, cursor)) {
