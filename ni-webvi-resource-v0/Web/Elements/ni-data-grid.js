...
(function () {
    ...
    var DROPDOWN_VAL_CONVERTER = NationalInstruments.HtmlVI.ValueConverters.DropDownValueConverter;
    ...
    var STRING_TEMPLATE_TYPE = 'JQX-MULTILINE-TEXT-BOX';
    var NUMERIC_TEMPLATE_TYPE = 'JQX-NUMERIC-TEXT-BOX';
    var CHECKBOX_TEMPLATE_TYPE = 'JQX-CHECK-BOX';
    var LED_TEMPLATE_TYPE = 'JQX-LED';
    var SLIDER_TEMPLATE_TYPE = 'JQX-SLIDER';
    var PROGRESSBAR_TEMPLATE_TYPE = 'JQX-PROGRESS-BAR';
    var DROPDOWN_TEMPLATE_TYPE = 'JQX-DROP-DOWN-LIST';
    var CENTERED_WIDGET_CONTAINER_CSS_CLASS = 'ni-widget-container-centered';
    ...
    var createJqxColumnWidget = function (dataGrid, row, column, value, cellElement) {
        if (!cellElement.firstElementChild) {
            ...
            var control = NI_SUPPORT.cloneControlElement(templateControl);
    ...
    var fireValueChanged = function (dataGrid, oldValue) {
    ...
    var addTemplateControlMutationObserver = function (dataGrid, column) {
        var observer, observerConfig, columnTypeHelper;
    ...
    var appendAddRowsToolbar = function (dataGrid) {
        ...
        rowCountInput.jqxNumberInput({ inputMode: 'simple', decimalDigits: 0, min: 1, max: 1000, value: 1, width: 55 });
        rowCountInput[0].style.width = '55px';
        rowCountInput[0].style.padding = '3px';
    ...
    columnTypeHelpers[STRING_TEMPLATE_TYPE] = {
        ...
        configureWidget: function (dataGrid, widgetElement) {
            widgetElement.addEventListener('change', function (evt) {
                if (evt.target === widgetElement) {
                    dataGrid.templateControlValueChanged(widgetElement, evt.detail.value);
                }
            });
            widgetElement.addEventListener('keydown', keyDownStopPropagation, false);
        },
        resize: function (element, size) {
            var verticalScrollBarVisibility = size.height >= 34 ? 'auto' : 'hidden';
    ...
    columnTypeHelpers[CHECKBOX_TEMPLATE_TYPE] = {
        ...
        configureWidget: function (dataGrid, widgetElement) {
            ...
            // Remove inline width=height=100% from widget, our CSS will set a correct/ smaller
            // size. That allows the control to be centered.
            widgetElement.style.removeProperty('width');
            widgetElement.style.removeProperty('height');
    ...
    columnTypeHelpers[DROPDOWN_TEMPLATE_TYPE] = {
        ...
        setWidgetValue: function (niElement, value, templateControl) {
            if (templateControl !== undefined) {
                var dataSource = (typeof templateControl.dataSource !== 'string') ? JSON.stringify(templateControl.dataSource) : templateControl.dataSource;
    ...
    class DataGrid extends NationalInstruments.HtmlVI.Elements.Visual {
        ...
        addEmptyRows(count) {
            var row, jqxRow, rows, i, j, n, column, oldLength, oldValue;
            if (count > 0) {
        ...
        updateJqxValueFromParsedValue() {
            ...
            var rowIndex, j, curRow, result;
            result = [];
            var changedRows = [];
            var dataGridFields = this.getColumnNames();
            for (rowIndex = 0; rowIndex < this.parsedValue.length; rowIndex++) {
                ...
                for (j = 0; j < dataGridFields.length; j++) {
                    var cellValue = this.parsedValue[rowIndex][dataGridFields[j]];
                    curRow[j] = cellValue;
                    if (rowIndex < this.jqxValue.length) {
                        var jqxCell = this.jqxValue[rowIndex]['' + j];
        ...
        attachedCallback() {
            var firstCall = super.attachedCallback(), widgetSettings, childElement, childColumns, jqref, that = this;
            if (firstCall === true) {
                this.niTypeInstance = new NIType(this.niType);
                ...
                jqref.on('cellendedit', function (event) {
                    var oldValue = that.deepCloneData();
        ...
        updateData(val) {
            ...
            var changedRows = this.updateJqxValueFromParsedValue();
            ...
            if (oldDataLength === this.parsedValue.length) {
                var jqxUpdatedValues = changedRows.map(function (row) {
        ...
        findInitialColumns() {
            var childColumns = [], columnItemName = NationalInstruments.HtmlVI.Elements.DataGridColumn.prototype.elementInfo.tagName.toUpperCase(), i;
        ...
        setDisabledOnChildElements() {
            var childTemplateElementSelector = 'ni-data-grid-column > *';
            var templateControlsToDisable = this.querySelectorAll(childTemplateElementSelector);
            templateControlsToDisable.forEach(templateControl => {
                templateControl.disabled = this.disabled;
            });
        }
        setDisabledOnAddRowsPane() {
            var elementSelectorToDisable = '.ni-row-count-text-field,.ni-add-rows-button';
            var elementsToDisable = this.querySelectorAll(elementSelectorToDisable);
        ...
        setDisabledOnPagingInput() {
            if (this.allowPaging) {
                var pagerInput = this.querySelectorAll('.jqx-grid-pager-input');
    ...
    NationalInstruments.HtmlVI.Elements.NIElement.defineElementInfo(DataGrid.prototype, 'ni-data-grid', 'HTMLNIDataGrid');
