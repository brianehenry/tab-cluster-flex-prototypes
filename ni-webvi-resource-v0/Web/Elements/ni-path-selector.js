...
(function () {
    ...
    var PATH_TYPE_ENUM = Object.freeze({
        ABSOLUTE: 'absolute',
        RELATIVE: 'relative'
    });
    // Static Private Functions
    var enablePopup = function (target) {
        var jqref = $(target.firstElementChild), jqrefPopoverContainer = $(target._popoverTag), jqrefPathControlButton;
    ...
    class PathSelector extends NationalInstruments.HtmlVI.Elements.Visual {
        ...
        addAllProperties(targetPrototype) {
            ...
            var proto = PathSelector.prototype;
        ...
        attachedCallback() {
            var firstCall = super.attachedCallback(), widgetSettings, childElement, jqref, button, that = this;
            if (firstCall === true) {
                widgetSettings = {};
        ...
        propertyUpdated(propertyName) {
            ...
            var childElement = this.firstElementChild, jqref = $(childElement);
            switch (propertyName) {
                case 'readOnly':
    ...
    NationalInstruments.HtmlVI.Elements.NIElement.defineElementInfo(PathSelector.prototype, 'ni-path-selector', 'HTMLNIPathSelector');
