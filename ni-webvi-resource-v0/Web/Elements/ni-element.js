...
(function () {
    ...
    var eventManager = (function () {
        var callbacks = [], fired = false, addCallback, notifyElementsRegistered, scheduleCallbackExecution;
    ...
    var attributeTable = (function () {
        var attrs = {}, addAttributeConfig = function (tagName, attributeName, attributeConfig) {
    ...
    class NIElement extends HTMLElement {
        ...
        static _finalizeObservedAttributes(elementClass) {
            var attrListFinal, attrListRaw, attrList;
        ...
        static _registerElements() {
            var toReg, elementClass;
            for (toReg in NationalInstruments.HtmlVI.Elements) {
                if (NationalInstruments.HtmlVI.Elements.hasOwnProperty(toReg)) {
                    elementClass = NationalInstruments.HtmlVI.Elements[toReg];
                    if (elementClass.prototype.elementInfo !== undefined) {
                        NIElement._finalizeObservedAttributes(elementClass);
                        window[elementClass.prototype.elementInfo.prototypeName] = elementClass;
                        window.customElements.define(elementClass.prototype.elementInfo.tagName, elementClass);
                    }
                }
            }
        ...
        createdCallback() {
            var attrList, currAttrName, currAttrConfig, currPropValue;
        ...
        connectedCallback() {
            ...
            if (this._attachedCallbackFirstCall === true) {
                var currAttrName, currAttrConfig, currPropValue, currAttrValue;
                var attrList = attributeTable.getAttributeList(this.elementInfo.tagName);
                for (currAttrName in attrList) {
                    if (attrList.hasOwnProperty(currAttrName)) {
                        currAttrConfig = attrList[currAttrName];
        ...
        addProperty(targetPrototype, config) {
            ...
            var propertyName = config.propertyName, defaultValue = config.defaultValue, defaultValueType = typeof defaultValue, fireEvent = config.fireEvent || false, addQuietProperty = config.addNonSignalingProperty || false, preventAttributeSync = config.preventAttributeSync || false, attributeName, fireEventName, privatePropertyName, quietPropertyName, createUpdatePropertyAction, typeArray, getter;
