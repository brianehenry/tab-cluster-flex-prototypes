    ...
    var createDataItemMaps = function (viModels) {
        ...
        for (var viModelName in viModels) {
            if (viModels.hasOwnProperty(viModelName)) {
                var viModel = viModels[viModelName], controlModels = viModel.getAllControlModels();
                ...
                for (var controlId in controlModels) {
                    if (controlModels.hasOwnProperty(controlId)) {
                        var controlModel = controlModels[controlId], editorRuntimeBindingInfo = controlModel.getEditorRuntimeBindingInfo();
                        var isVisualModel = editorRuntimeBindingInfo !== undefined;
                        var isBoundControl = isVisualModel && editorRuntimeBindingInfo.dataItem !== undefined;
                        var isTopLevelControl = isBoundControl && editorRuntimeBindingInfo.dataItem !== '';
