...
class NIEventRegistrationService {
    ...
    getEventOracleIndex(controlModel) {
        ...
        if (localBindingInfo !== undefined && this.eventOracleMaps.has(localBindingInfo.encodedVIName)) {
            const controlIdToEventOracleIndex = this.eventOracleMaps.get(localBindingInfo.encodedVIName);
            const controlId = parseInt(controlModel.niControlId, 10);
            if (controlIdToEventOracleIndex.has(controlId)) {
                return controlIdToEventOracleIndex.get(controlId);
            }
        }
        return -1;
    }
...
var EventIds;
(function (EventIds) {
    EventIds[EventIds["ValueChanged"] = 2] = "ValueChanged";
})(EventIds || (EventIds = {}));
// @ts-ignore
NationalInstruments.HtmlVI.NIEventRegistrationService = NIEventRegistrationService;
NationalInstruments.HtmlVI.EventIds = EventIds;
