...
(function (parent) {
    ...
    proto.addFrontPanelControlModel = function (element) {
        ...
        var controlElements = NI_SUPPORT.queryAllControlsByNIControlId(element.viRef, element.niControlId);
        ...
        var currParent, parentModel = this;
    ...
    proto.processControlUpdateToSetGPropertyValue = function (controlId, gPropertyName, gPropertyValue) {
        var controlModel = this.controlModels[controlId];
        if (controlModel !== undefined) {
            controlModel.setGPropertyValue(gPropertyName, gPropertyValue);
        }
        else {
            throw new Error('Cannot find the control model with id (' + controlId + ') to update the value of (' + gPropertyName + ')');
        }
    };
    proto.processControlUpdateToGetGPropertyValue = function (controlId, gPropertyName) {
        var controlModel = this.controlModels[controlId];
        if (controlModel !== undefined) {
            return controlModel.getGPropertyValue(gPropertyName);
        }
        else {
            throw new Error('Cannot find the control model with id (' + controlId + ') to get value of (' + gPropertyName + ')');
        }
    };
    proto.controlChanged = function (controlModel, propertyName, newValue, oldValue) {
    ...
    proto.getNameVireoEncoded = function () {
        return NationalInstruments.Vireo.Vireo.encodeIdentifier(this.viName);
    };
    proto.enableEvents = function () {
        let webAppModel = this.getOwningWebApplication();
