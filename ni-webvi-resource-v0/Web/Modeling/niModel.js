...
(function () {
    ...
    NationalInstruments.HtmlVI.Models.NIModel = function () {
        ...
        for (currProp in _propertyTable) {
            if (_propertyTable.hasOwnProperty(currProp) && _propertyTable[currProp].computedProp === false) {
                this[_propertyTable[currProp].privatePropName] = DEEP_COPY_CONVERTER.deepCopy(_propertyTable[currProp].defaultValue);
            }
        }
    ...
    var validateAddModelPropertyArgs = function (targetPrototype, config) {
        ...
        var defaultValue = DEEP_COPY_CONVERTER.deepCopy(config.defaultValue);
    ...
    proto.addModelProperty = function (targetPrototype, config) {
        ...
        var privatePropertyName = validConfig.privatePropertyName, defaultValue = validConfig.defaultValue, propertyName = validConfig.propertyName, customSetter = validConfig.customSetter, customGetter = validConfig.customGetter, computedProp = validConfig.computedProp;
    ...
    proto.setGPropertyValue = function (gPropertyName, gPropertyValue) {
        throw new Error("Property " + gPropertyName + " does not exist");
    };
